 ...
 * Shared by OpenCode V1 (ai-sdk + native LLM) and the V2 SessionRunner.
 *
...
/**
 * Identical empty-success (tsc clean, rm, mkdir) — refuse the 2nd rematch.
 * Intervening read/grep/glob must not reset this; only a mutating tool does.
 */
export const EMPTY_SUCCESS_REFUSE_THRESHOLD = 2
/** Force text-only pivot on the 3rd identical empty-success rematch. */
export const EMPTY_SUCCESS_PIVOT_THRESHOLD = 3

/** Model-facing rewrite for successful commands with empty stdout. */
export const EMPTY_SUCCESS_TOOL_OUTPUT =
  "(no output) — command succeeded with empty stdout (exit 0). Do not retry this command; continue with the next distinct step."
...
export type ToolLoopDecision = {
  ...
  pivot: boolean
  count: number
  reason?: "streak" | "oscillation" | "fake_progress" | "banned" | "empty_success"
}
...
/** Empty / "(no output)" / enriched success — Laguna treated silence as failure. */
export function isEmptySuccessOutput(content: string): boolean {
  const c = (content || "").trim()
  if (!c) return true
  if (/^\(?\s*no output\s*\)?/i.test(c)) return true
  if (/^command succeeded/i.test(c)) return true
  if (/empty stdout/i.test(c) && /succeed/i.test(c)) return true
  return false
}

function isEmptySuccessInvocation(inv: ToolInvocation): boolean {
  return inv.status === "completed" && isEmptySuccessOutput(inv.output ?? "")
}

function isPassiveTool(name: string): boolean {
  return PASSIVE_TOOLS.has(name.toLowerCase())
}
...
export function outputFingerprint(inv: Pick<ToolInvocation, "output" | "status">): string {
  ...
  if (isEmptySuccessOutput(inv.output ?? "")) return "(empty)"
  const t = (inv.output ?? "").replace(/\s+/g, " ").trim().slice(0, 500)
  return t || "(empty)"
}

/**
 * Same command with empty success, skipping inspect-only tools in between.
 * `edit` / `write` / a different bash command resets the count.
 */
export function countSameKeyEmptySuccess(
  ...
  for (let i = history.length - 1; i >= 0; i--) {
    ...
    if (toolLoopKeysRelated(nextKey, key)) {
      if (isEmptySuccessInvocation(inv)) {
        count++
        continue
      }
      break
    }
    if (isPassiveTool(inv.tool)) continue
    break
  }
  return count + 1
}
...
/**
 * Drop raw tool XML that leaked into assistant text (Hermes `<tool_call>`, Gemma, North).
 * Incomplete open tags are withheld so the user never sees a half-written call.
 */
export function stripLeakedToolMarkup(text: string): string {
  ...
  out = out.replace(/<\|START_ACTION\|>[\s\S]*?<\|END_ACTION\|>/gi, "")
  out = out.replace(/<tool_call\b[\s\S]*$/i, "")
  out = out.replace(/<\|tool_call[\s\S]*$/i, "")
  out = out.replace(/<\|START_ACTION\|>[\s\S]*$/i, "")
  return out
}
...
export function evaluateToolLoop(
  ...
  const emptySuccess = countSameKeyEmptySuccess(history, next)
  ...
  if (emptySuccess >= EMPTY_SUCCESS_REFUSE_THRESHOLD && emptySuccess >= count) {
    count = emptySuccess
  ...
  const refuse =
    ...
    emptySuccess >= EMPTY_SUCCESS_REFUSE_THRESHOLD
  const pivot =
    streak >= pivotAt ||
    osc >= TOOL_LOOP_PIVOT_THRESHOLD ||
    sameKey >= TOOL_LOOP_FAKE_PROGRESS_CAP ||
    emptySuccess >= EMPTY_SUCCESS_PIVOT_THRESHOLD
...
/** V2 runner messages (`type: "assistant"` + `content[]` tool parts). */
export function toolInvocationsFromV2Messages(
  messages: ReadonlyArray<{ type?: string; content?: ReadonlyArray<unknown> }>,
): ToolInvocation[] {
  const out: ToolInvocation[] = []
  for (const msg of messages) {
    ...
    for (const part of msg.content ?? []) {
      ...
      const status: ToolInvocation["status"] = state.status === "error" ? "error" : "completed"
      out.push({
        tool,
        input: state.input ?? {},
        output: status === "completed" ? v2ToolOutput(state) : undefined,
        status,
      })
    }
  }
